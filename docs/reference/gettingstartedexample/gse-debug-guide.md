...
| -52018 | Bad packet size detected on SI<0...7>. Transfer aborted due to data corruption as a result of a packet size that did not match the expected value. | The packet payload was larger or smaller than the packet header indicated. | Verify the camera is powered properly and connections on the serial input channel are correct. |
...
| -375704 |  Display channel number <0...7> is invalid | The display channel selected is not between 0 and 7 | Make sure the channel number is between 0 and 7 and select a channel that has been configured in the FPGA for display. |
